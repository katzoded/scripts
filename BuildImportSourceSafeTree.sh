...
| ~/dev-newton/scripts/NoFileCreationReplaceFileList.sh "$\(.*\)\r" "ss Dir ${SOURCESAFE_SUB_TREE}/\1 -F- -Yokatz,Johanson23 | grep -v item | grep -v : | ~/dev-newton/scripts/NoFileCreationReplaceFileList.sh '$\\\\(.*\\\\)\\\\r' './ImportSourceSafeIntoGit.sh ${SOURCESAFE_DB} ${SOURCESAFE_SUB_TREE_SLASHED}\1 \\\\1 '"    |sh
