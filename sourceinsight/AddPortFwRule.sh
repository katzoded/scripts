...
iptables -t nat -A PREROUTING -d ${LOCAL_INTERFACE_IP} -p ${PROTOCOL} -m ${PROTOCOL} --dport ${DEST_PORT} -j DNAT --to-destination ${PBX_IP}
...
iptables -t nat -A POSTROUTING -d ${PBX_IP}/32 -p ${PROTOCOL} -m ${PROTOCOL} --dport ${DEST_PORT} -j SNAT --to-source ${LOCAL_INTERFACE_IP}    
