...
if [ -f ${ROOT_PATH}/checkout.log ]; then
	if grep ${FILE_NAME} ${ROOT_PATH}/checkout.log; then
		echo "File exist in checkout list Do: uncheck out"
	else
...
if [ -f ${FILE_NAME} ]; then
	res=$(ssh ${REPLACE_WITH_USER}@${SERVER} "cleartool unco ${REPLACED_FILE_NAME}") 	
...
echo ${FILE_NAME} >> ${ROOT_PATH}/uncheckout.log
exit;
